...
class User(db.Model, UserMixin):
    ...
    def set_role(self):
        if self.role is None:
            if self.username == current_app.config['ALBUMY_ADMIN_USERNAME']:
    ...
    def is_following(self, user):
        if user.id is None:
            return False
        return Follow.query.with_parent(self).filter_by(followed_id=user.id).first() is not None
...
class Notification(db.Model):
    ...
    is_read = db.Column(db.Integer, default=False)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow, index=True)
